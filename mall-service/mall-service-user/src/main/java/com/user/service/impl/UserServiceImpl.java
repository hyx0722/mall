...
public class UserServiceImpl  implements UserService {
    ...
    public PageBean<User> pageUsers(Integer page, Integer size, String keyword) {
        ...
        long total = userMapper.countUsers(kw);
        if (total == 0L) {
            return new PageBean<>(0L, List.of());
        }
        List<User> items = userMapper.pageUsers(kw, (p - 1) * s, s);
    ...
    public void adminUpdate(Long operatorId, Long id, Integer status, Integer role, String email, String phone) {
        ...
        User target = userMapper.findUserById(id);
        if (target == null) {
        ...
        try {
            int affected = userMapper.adminUpdateFields(id, status, role, email, phone);
            if (affected == 0) {
    ...
    public void adminResetPwd(Long operatorId, Long id, String newPassword) {
        ...
        int affected = userMapper.updatePasswordById(id, passwordEncoder.encode(newPassword));
        if (affected == 0) {
            throw new BusinessException("用户不存在");
        }
    }
    ...
    public void seedAdmin(String username, String password) {
        userMapper.insertAdmin(username, passwordEncoder.encode(password));
    }
